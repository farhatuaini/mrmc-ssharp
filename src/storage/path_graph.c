...
path_graph* get_new_path_graph(int size)
{
	int i;
	path_graph* pg;
        pg = (path_graph *) calloc((size_t) size, sizeof(path_graph));
	for(i=0;i<size;i++)
...
/* SLOW */
double get_prob(const path_graph * pg, int state, double reward)
{
...
const path_graph_ele * get_path_graph_ele(const path_graph * pg, int state)
{
	return pg[state].pge;
}

/* Get the number of path graph elements for the given state */
int get_path_graph_num(const path_graph * pg, int state)
{
	return pg[state].num;
}
...
/* For Test Only */
void print_path_graph(const path_graph * pg, int size)
{
	int i, j, num;
        const
